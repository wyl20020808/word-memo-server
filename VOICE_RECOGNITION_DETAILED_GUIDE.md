...
2. 点击右上角"登录"按钮
3. 如果没有百度账号，点击"注册"创建新账号

### 1.2 创建应用

1. 登录后，点击页面右上角的"控制台"
2. 在左侧菜单中找到"应用列表"，点击进入
3. 点击蓝色的"创建应用"按钮
4. 在弹出的表单中填写：
   - **应用名称**：输入任意名称，例如 "微信小程序语音识别"
   - **应用类型**：选择 "服务端"
   - **应用描述**：可选，输入描述信息
5. 点击"立即创建"按钮
...
1. 应用创建成功后，会显示应用详情页面
2. 在页面中找到以下信息：
   - **API Key**：一串字母和数字的组合，例如 `abc123def456`
   - **Secret Key**：另一串字母和数字的组合，例如 `xyz789uvw012`
3. **重要**：复制这两个值，保存到安全的地方（记事本或密码管理器）

### 1.4 检查配额

1. 在应用详情页面，找到"配额"或"使用情况"部分
2. 查看免费配额：
   - 免费版通常每天有 500 次调用次数
   - 如果需要更多，可以升级为付费版本
...
1. 启动微信开发者工具
2. 打开你的小程序项目
3. 确保项目已开通云开发（如果没有，需要先开通）

### 2.2 进入云开发面板

1. 在微信开发者工具顶部菜单栏，找到"云开发"选项卡
2. 点击"云开发"进入云开发面板
3. 在左侧菜单中，找到"云函数"选项
...
4. 代码复制完成后，按 `Ctrl+S`（或 `Cmd+S`）保存

### 2.5 配置环境变量

1. 在编辑器上方，找到"函数配置"按钮，点击打开
2. 在弹出的配置面板中，找到"环境变量"部分
